...
- [ ] 2.7 [Build] Create/use an Ansible role to manage infrastructure
  - **Collection correction (2026-09-24):** the v1.1 Equipment and Software List names only `cisco.aci 2.12.0` under Ansible Core 2.19.3 — no `cisco.ios`. Confirmed live via `ansible-galaxy collection list` on the CWS: only `ansible.netcommon 8.4.0`, `ansible.utils 6.0.1`, `cisco.aci 2.12.0` are installed. Target platform switched from IOSv/IOSvL2/Cat8000V to APIC/ACI, same pivot as 2.8 — same tenant/AP/EPG/contract object model, worth comparing the two workbooks. **Known gap:** `cisco.aci` only provides an HTTPAPI connection to APIC; 2.7.d also lists `network_cli` and `netconf` legs, and nothing in the installed collection set can exercise those against this equipment (APIC has no NETCONF agent or automatable config-mode CLI). The workbook itself doesn't call this out — tracked here instead. If the actual exam expects hands-on `network_cli`/`netconf` Ansible connection work, that surface needs practice elsewhere (e.g. raw netconf-console/RESTCONF curl work already covered under 2.4–2.6, just not via Ansible).
  - [ ] 2.7.a Loop control
  - [ ] 2.7.b Conditionals
...
### Lab Infrastructure (lab_infra/, staged 2026-09-19, deployed 2026-09-20)
