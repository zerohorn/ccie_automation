...
| `2-7-ansible.html` | 2.7 Ansible | Not started (workbook built, hands-on pending). **Retargeted 2026-09-24:** `cisco.ios` is not on the equipment list and confirmed absent from the CWS (`ansible-galaxy collection list` shows only `ansible.netcommon`, `ansible.utils`, `cisco.aci`). Workbook now targets APIC/ACI via `cisco.aci` throughout — same tenant/AP/EPG/contract model as `2-8-terraform.html`. `2.7.d`'s `network_cli`/`netconf` connection-plugin legs can't be exercised with this collection set (APIC only supports HTTPAPI); that gap is tracked in `blueprint-tracker.md`, not called out in the workbook itself. Not yet verified against a live APIC. |
