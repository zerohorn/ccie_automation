...
| `2-7-ansible.html` | 2.7 Ansible | Not started (workbook built, hands-on pending). **Corrected 2026-09-21:** `cisco.ios` is not on the equipment list and confirmed absent from the CWS (`ansible-galaxy collection list` shows only `ansible.netcommon`, `ansible.utils`, `cisco.aci`). CLI leg of `2.7.d` now uses `ansible.netcommon.cli_command`/`cli_config` with the generic `ansible.netcommon.default` cliconf; NETCONF/HTTPAPI legs unchanged since they never used `cisco.ios`. The generic-cliconf approach is unverified against a live device — check it first. |
